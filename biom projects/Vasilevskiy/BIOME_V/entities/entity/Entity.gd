...
func give_birth(parent_plant,children,children_scene):
	for i in range(children):
		...
		var pos = Vector2()	
		pos.x = parent_plant.position.x + randf_range(-60,60) * children * (parent_plant.scale.x*1.25)
		pos.y = parent_plant.position.y + randf_range(-60,60) * children * (parent_plant.scale.x*1.25)
		...
		baby.velocity.x = 0
		baby.velocity.y = 0
		baby.scale = Vector2(0.3,0.3)
		var type = parent_plant.tree_type
		if type == "green":
			baby.anim.play("default")
		if type == "pink":
			baby.anim.play("sakura")
		if type == "orange":
			baby.anim.play("orange")
