...
func _process(delta):
	...
	$CanvasLayer/AgeCount.text = "It's been " + str(round(Global.age_of_the_world)) + " years since the start"
...
func _on_button_speed_up_toggled(button_pressed):
	Global.speed_up = 2.0 if Global.speed_up == 1.0 else 1.0 if Global.speed_up == 64.0 else 4.0 if Global.speed_up == 2.0 else 8.0 if Global.speed_up == 4.0 else 16.0 if Global.speed_up == 8.0 else 32.0 if Global.speed_up == 16.0 else 64.0
...
func spawn_all(predators,grass_eaters,plants,food):
	for i in range(predators):
		spawn_entity(predator_scene,randf_range(-50,50),randf_range(-50, 50))
	for i in range(grass_eaters):
		spawn_entity(grass_eater_scene,randf_range(-50,50),randf_range(-50, 50))
	for i in range(plants):
		spawn_entity(plant_scene,randf_range(-50,50),randf_range(-50, 50))
	for i in range(food):
		spawn_entity(food_scene,randf_range(-50,50),randf_range(-50, 50))
